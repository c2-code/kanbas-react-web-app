...
export default function AssignmentEditor() {
  const { cid, aid } = useParams(); // Fetch aid from the URL
  const navigate = useNavigate(); 
  const assignment = db.assignments.find((item) => item._id === aid); // Find the specific assignment by aid

  // If no assignment is found, return a fallback message
  if (!assignment) {
    ...
    <div id="wd-assignments-editor" className="container">
      ...
      <div className="row mb-3">
        ...
        <div className="col-md-8">
          <input id="wd-points" value={assignment.points} className="form-control w-25" readOnly />
        </div>
      </div>
